...
class ClassifyProcess:
    def __init__(self, *args):
        ...
        self.rabbitmq_manager.create_or_connect_queue(
            self.catalog_queue, self.catalog_route
        )
    ...
    def classify_records(
        ...
        if start_datetime:
            get_unfrbrized_records_query = get_unfrbrized_records_query.filter(
                Record.date_modified > start_datetime
            )
        ...
        while unfrbrized_record := get_unfrbrized_records_query.first():
            ...
            if (
                self.params.limit
                and self.record_buffer.ingest_count >= self.params.limit
            ):
                break
        ...
        for id in queryable_ids:
            ...
            try:
                self.classify_record_by_metadata(
                    identifier, identifier_type, author, record.title
                )
            except Exception:
    ...
    def classify_record_by_metadata(self, identifier, identifier_type, author, title):
        search_query = self.oclc_catalog_manager.generate_search_query(
            identifier, identifier_type, title, author
        )
        ...
        for related_oclc_bib in related_oclc_bibs:
            ...
            related_oclc_numbers = self.oclc_catalog_manager.get_related_oclc_numbers(
    ...
    def get_oclc_catalog_records(self, identifiers):
        ...
        for oclc_number, uncached in cached_oclc_numbers:
            if not uncached:
                logger.debug(
                    f"Skipping catalog lookup process for OCLC number {oclc_number}"
                )
                continue
            ...
            self.rabbitmq_manager.send_message_to_queue(
                self.catalog_queue,
                self.catalog_route,
                {"oclcNo": oclc_number, "owiNo": owi_number},
            )
    ...
    def _get_queryable_identifiers(self, identifiers):
        return list(
            filter(
                lambda id: re.search(r"\|(?:isbn|issn|oclc)$", id) != None, identifiers
            )
        )
