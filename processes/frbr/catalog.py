...
class CatalogProcess:
    ...
    def process_catalog_messages(self, max_attempts: int = 4):
        for attempt in range(0, max_attempts):
            ...
            while message := self.rabbitmq_manager.get_message_from_queue(
                ...
                self.rabbitmq_manager.acknowledge_message_processed(
                    message_props.delivery_tag
                )
    ...
    def process_catalog_message(self, message_body):
        ...
        if not catalog_record:
            logger.warning(
                f"Unable to get catalog record for OCLC number {oclc_number}"
            )
            return
    ...
    def parse_catalog_record(self, catalog_record, oclc_number, owi_number):
        try:
            ...
            logger.exception(
                f"Unable to parse OCLC catalog MARC XML for OCLC number: {oclc_number}"
            )
            return

        catalog_record_mapping = CatalogMapping(
            parsed_marc_xml, {"oclc": "http://www.loc.gov/MARC21/slim"}, {}
        )
        ...
        try:
            ...
            logger.exception(
                f"Unable to map OCLC catalog record for OCLC number: {oclc_number}"
            )
