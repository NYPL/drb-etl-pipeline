...
class LOCProcess:
    ...
    def runProcess(self):
        ...
        for record in records:
            self.s3_manager.store_pdf_manifest(
                record=record, bucket_name=self.s3_bucket
            )
    ...
    def store_epub(self, record: Record):
        ...
        epub_part = next(
            filter(lambda part: part.file_type == "application/epub+zip", record.parts),
            None,
        )

        if epub_part is not None:
            epub_location = f"epubs/{epub_part.source}/{record_id}.epub"
            epub_url = get_stored_file_url(
                storage_name=self.s3_bucket, file_path=epub_location
            )
            ...
            self.rabbitmq_manager.send_message_to_queue(
                self.file_queue,
                self.file_route,
                get_file_message(epub_part.url, epub_location),
            )
