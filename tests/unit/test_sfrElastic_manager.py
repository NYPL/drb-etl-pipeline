...
class TestSFRElasticRecordManager:
    ...
    def testDBWork(self, mocker):
        ...
        testWork.subjects = [{'sub': 'Subject 1'}, {'sub': 'Subject 2'}, {'sub': 'Subject 3'}]
        testWork.authors = ['Author 1', 'Author 2']
        testWork.contributors = ['Contributor 1']
        testWork.identifiers = [
            [('type', 'test'), ('iden', 'id1')],
            [('type', 'test'), ('iden', 'id2')]
        ]
        testWork.languages = [{'lang': 'Language 1'}]
    ...
    def testDBEdition(self, mocker):
        ...
        testEdition.languages = [{'lang': 'Lang 1'}, {'lang': 'Lang 2'}]
    ...
    def test_enhanceWork(self, testInstance, mocker):
        ...
        mockSubject = mocker.patch('managers.sfrElasticRecord.ESSubject')
        mockSubject.side_effect = [list(d.values())[0] for d in testInstance.dbWork.subjects]
        mockAgent = mocker.patch('managers.sfrElasticRecord.ESAgent')
        mockAgent.side_effect = testInstance.dbWork.authors + testInstance.dbWork.contributors
        mockIdentifier = mocker.patch('managers.sfrElasticRecord.ESIdentifier')
        mockIdentifier.side_effect = [i[1][1] for i in testInstance.dbWork.identifiers]
        mockLanguage = mocker.patch('managers.sfrElasticRecord.ESLanguage')
        mockLanguage.side_effect = [list(d.values())[0] for d in testInstance.dbWork.languages]
        ...
        assert testInstance.work.alt_titles == ['Alt Title 1', 'Alt Title 2']
        assert testInstance.work.subjects == ['Subject 1', 'Subject 2', 'Subject 3']
        assert testInstance.work.agents == ['Author 1', 'Author 2', 'Contributor 1']
        assert testInstance.work.identifiers == ['id1', 'id2']
        assert testInstance.work.languages == ['Language 1']
    ...
    def test_createEdition(self, testInstance, testDBEdition, mocker):
        ...
        assert testEdition.title == 'Test Title'
