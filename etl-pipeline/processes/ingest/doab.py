...
class DOABProcess:
    ...
    def __init__(self, *args):
        ...
        self.dspace_service = DSpaceService(
            base_url=self.DOAB_BASE_URL, source_mapping=DOABMapping
        )

    def runProcess(self):
        try:
            if self.params.record_id is not None:
                record = self.dspace_service.get_single_record(
    ...
    def manage_links(self, record: Record):
        ...
        for epub_link in link_manager.epub_links:
            ...
            self.rabbitmq_manager.send_message_to_queue(
                self.file_queue, self.file_route, get_file_message(epub_uri, epub_path)
            )
