...
def frbrized_record_data(db_manager, test_title, test_subject, test_language):
    ...
    cluster_process = ClusterProcess(
        "complete", None, None, str(test_frbrized_record_data["uuid"]), None
    )
...
def unfrbrized_record_uuid(db_manager):
    ...
    unfrbrized_record = create_or_update_record(
        record_data=test_unfrbrized_record_data, db_manager=db_manager
    )
