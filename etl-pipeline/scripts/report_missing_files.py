...
def is_record_missing_files(record_parts: tuple) -> bool:
    ...
    for part in parts:
        ...
        if file_part.file_bucket:
            try:
                s3_manager.s3Client.head_object(
                    Bucket=file_part.file_bucket, Key=file_part.file_key
                )
            ...
            try:
                url_head_response = requests.head(
                    url,
                    headers={
...
def main(*args):
    ...
    record_parts = (
        db_manager.session.query(Record.has_part).filter(Record.source == source).all()
    )
    ...
    def track_progress(is_missing_file):
        with lock:
            ...
            print(
                f"{source} records with missing files: {missing_file_count.value}/{record_count.value} - total: {len(record_parts)}",
                end="\r",
                flush=True,
            )

    with Pool(processes=8) as pool:
        for is_missing_file in pool.imap_unordered(
            is_record_missing_files, record_parts
        ):
            track_progress(is_missing_file)

    print(
        f"{source} records with missing files: {missing_file_count.value}/{len(record_parts)}"
    )
