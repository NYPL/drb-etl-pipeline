...
class PublisherBacklistService(SourceService):
        ...
        for record in records:
            try:
                ...
                try:
                    ...
                    logger.error(
                        f'Could not extract a Drive identifier from {record_metadata.get("DRB_Record ID")}'
                    )
                    continue
                ...
                if not file:
                    logger.error(
                        f'Failed to retrieve file for {record_metadata.get("DRB_Record ID")} from Google Drive'
                    )
                    continue
                ...
                s3_response = self.s3_manager.putObjectInBucket(
                    file.getvalue(), s3_path, bucket
                )

                if not s3_response.get("ResponseMetadata").get("HTTPStatusCode") == 200:
                    logger.error(
                        f'Failed to upload file for {record_metadata.get("DRB_Record ID")} to S3'
                    )
                    continue
                ...
                self.add_has_part_mapping(
                    ...
                    record_permissions["is_downloadable"],
                    record_permissions["requires_login"],
                )
    ...
    def add_has_part_mapping(
        self, s3_url: str, record: Record, is_downloadable: bool, requires_login: bool
    ):
        item_no = "1"
        ...
        record.has_part.append(
            "|".join([item_no, s3_url, record.source, media_type, json.dumps(flags)])
        )
