...
class LOCMapping(RecordMapping):
    def __init__(self, source):
        self.record = self._map_to_record(source)
    
    def _map_to_record(self, source_record: dict) -> Record:
        ...
        return Record(
            ...
            medium=self._get_medium(source_record),
            ...
            publisher=self._parse_publishers(item_details.get('created_published')),
            ...
            has_part=self._get_has_part(source_record)
        )
    
    def _get_medium(self, source_record: dict) -> Optional[str]:
    ...
    def _parse_publishers(self, created_published_list: list[str]) -> Optional[list[str]]:
        publishers = []
    ...
    def _get_has_part(self, source_record: dict) -> list[str]:
        has_part = []
        first_file_resource = source_record['resources'][0]
